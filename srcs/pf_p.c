...
/*   Updated: 2020/02/09 17:20:54 by thbeaumo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_struct	handle_flag_p(unsigned long int v, t_struct datas, char *tab, int l)
{
	...
	datas = (datas.flag[prec] == '.' && datas.flag[size_prec] <= 0 && v == 0)
		? datas : putnbr_base(v, tab, datas);
	if (datas.flag[flags] == '-')
		while (datas.flag[size]-- > l)
			datas = ft_buffer(' ', datas);
	return (datas);
...
t_struct	handle_p(unsigned long int val, int tmp, t_struct datas, char *tab)
{
	int len;
	...
	else
	{
		...
		while ((tmp-- - len) > 0)
			datas = ft_buffer('0', datas);
		datas = (datas.flag[prec] == '.' && datas.flag[size_prec] <= 0 &&
				val == 0) ? datas : putnbr_base(val, tab, datas);
	}
	return (datas);
}
...
t_struct	pf_p(va_list ap, t_struct datas, int i, const char *s)
{
	...
	datas = handle_p(val, tmp, datas, tab);
	return (datas);
}
